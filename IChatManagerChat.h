...
- (TMBMessage *)sendMessage:(TMBMessage *)message
 ...
 */
- (TMBMessage *)asyncSendMessage:(TMBMessage *)message
                      progress:(id<ITMBChatProgressDelegate>)progress;
...
- (TMBMessage *)asyncSendMessage:(EMMessage *)message
...
/*!
 @method
 @brief 同步方法,删除本地数据库的消息
 @discussion     删除数据库所保存的文字消息,另一些图片、文件包含附件的消息不会删除相应的已下载的资源
 @param message  消息对象(包括from, to, body列表等信息)
 @result 删除成功返回YES, 失败返回NO
 */
- (BOOL)syncRemoveMessage:(TMBMessage *)message;

/*!
 @method
 @brief 重新设置发送消息时的进程代理对象
 @discussion 应用场景: 发送文件消息，用户退出聊天界面，重新加载数据时，通过 message.deliveryState 的判断，如果是正在发送的状态，可以调用此方法，设置新的文件上传进度代理
 @result YES：设置成功.
 */
-(BOOL)asyncResetProgress:(id<ITMBChatProgressDelegate>)progress
               forMessage:(TMBMessage *)message;
...
/*!
 @method
 @brief 异步方法, 下载文件消息里的对应的文件url
 @discussion     将会判断消息体的类型，如果是属于文件类别的，将会启动下载线程下载，可在代理progress查看进度,可以回调@selector() 监听下载状态
 @param message  消息对象(包括from, to, body列表等信息)
 @param filePath 文件下载缓存绝对路径(需要保证此地址的唯一性)
 @param progress 发送多媒体信息时的progress回调对象
 @result
 */
- (void) asyncDownloadFileWithMessage:(TMBMessage *)message
                          downladPath:(NSString*) filePath
                             progress:(id<ITMBChatProgressDelegate>)progress;
...
- (TMBMessage *)resendMessage:(TMBMessage *)message
                        progress:(id<ITMBChatProgressDelegate>)progress
                           error:(EMError **)pError;
 */
...
/*!
 ...
 @result 发送的消息对象(因为是异步方法, 不能作为发送完成或发送成功失败与否的判断)
 */
- (TMBMessage *)asyncResendMessage:(TMBMessage *)message
                             progress:(id<ITMBChatProgressDelegate>)progress;
...
- (EMMessage *)forwardMessage:(EMMessage *)message
                     ...
                     progress:(id<IEMChatProgressDelegate>)progress
                        error:(EMError **)pError TMB_DEPRECATED_IOS(2_0_0, 2_0_9, "请使用带 ext 的方法");
*/
/*!
 @method
...
- (EMMessage *)asyncForwardMessage:(EMMessage *)message
                                    to:(NSString *)username 
                            isGroup:(BOOL)isGroup
                              progress:(id<IEMChatProgressDelegate>)progress TMB_DEPRECATED_IOS(2_0_0, 2_0_9, "请使用带 ext 的方法");
*/
/*!
 @method
 @brief 异步方法, 将某一条消息转发给另一个聊天用户
...
- (EMMessage *)asyncForwardMessage:(EMMessage *)message 
                            ...
                            completion:(void (^)(EMMessage *message,
                                                 EMError *error))completion
                               onQueue:(dispatch_queue_t)aCompletionQueue TMB_DEPRECATED_IOS(2_0_0, 2_0_9, "请使用带 ext 的方法");
*/
/*!
 @method
 @brief 将某一条消息转发给另一个聊天用户
